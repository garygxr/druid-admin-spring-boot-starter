  ...
  var isOrderRequest = false;
	...
	return  {
		...
		buildHead : function(index) {
			...
			var instanceId = druid.common.getUrlVar("instanceId");
			$.get('header.html',function(html) {
				html = html.replace(new RegExp(".html","g"), '.html?serviceId=' + serviceId + '&instanceId=' + instanceId);
		...
		ajaxRequestForReset : function() {
			...
			$.ajax({
				type: 'POST',
				url: "reset-all.json?serviceId=" + serviceId + '&instanceId=' + instanceId,
		...
		ajaxRequestForLogAndReset : function() {
			...
			$.ajax({
				type: 'POST',
				url: "log-and-reset.json?serviceId=" + serviceId + '&instanceId=' + instanceId,
				success: function(data) {
					if (data.ResultCode == 1) {
						alert("already reset all stat");
					}
		...
		getAjaxUrl : function(uri) {
			var serviceId =  druid.common.getUrlVar("serviceId");
	    	var instanceId = druid.common.getUrlVar("instanceId");
			var result = uri + 'serviceId=' + serviceId + '&instanceId=' + instanceId + '&';
